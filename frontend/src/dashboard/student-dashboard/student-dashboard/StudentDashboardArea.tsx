...
const StudentDashboardArea = () => {
  ...
  const handleBookingCancel = async (bookingId: string, scheduledAt: string) => {
    ...
    if (hoursDiff < 24) {
      toast.error(t('student_dashboard.toast.cancel_warning'));
      return;
    }

    if (window.confirm(t('common.are_you_sure') + ' ' + t('student_dashboard.booking_cancelled_propmt', { defaultValue: "You won't be able to revert this!" }))) {
      try {
        await cancelBooking(bookingId, token);
        toast.success(t('student_dashboard.booking_cancelled_success'));
        fetchBookings(); // Refresh data
      } catch (err: any) {
        toast.error(err.message || t('student_dashboard.action_failed'));
      }
    }
  };
  ...
  const handleReschedule = async (bookingId: string) => {
    ...
    const hoursDiff = (lessonDate.getTime() - now.getTime()) / (1000 * 60 * 60);
    if (hoursDiff < 24) {
      toast.error(t('student_dashboard.toast.reschedule_warning'));
      return;
    }
    ...
    try {
      ...
      toast.error(err.message || t('student_dashboard.toast.fetch_slots_error'));
  ...
  const handleConfirmReschedule = async () => {
    ...
    try {
      ...
      toast.success(t('student_dashboard.toast.reschedule_success'));
      setRescheduleBookingId(null);
      fetchBookings();
    } catch (err: any) {
      toast.error(err.message || t('student_dashboard.toast.reschedule_error'));
    } finally {
  ...
  const handleShareLink = async (booking: Booking) => {
    ...
    if (!meetingUrl) {
      toast.error(t('student_dashboard.toast.no_link'));
      return;
    }

    try {
      await navigator.clipboard.writeText(meetingUrl);
      toast.success(t('student_dashboard.toast.link_copied'));
    } catch (err) {
      ...
      try {
        ...
        toast.error(t('student_dashboard.toast.copy_failed'));
      }
                      ...
                      <div className="d-flex gap-2">
                        {nextLesson.meeting?.joinUrlStudent && (
                          ...
                          >
                            ...
                            {t('student_dashboard.join_lesson')}
                          </a>
                        )}
                            ...
                            <div className="dropdown-menu show" style={{ position: 'absolute', right: 0, top: '100%', marginTop: '5px', zIndex: 1000, minWidth: '180px', boxShadow: '0 5px 15px rgba(0,0,0,0.1)', padding: '8px 0', border: 'none', borderRadius: '8px' }}>
                              {nextLesson.meeting?.joinUrlStudent && (
                                ...
                                >
                                  ...
                                  <span>{t('student_dashboard.share_link')}</span>
                                </button>
                              )}
                              <button
                                className="dropdown-item d-flex align-items-center gap-2 px-3 py-2"
                    ...
                    <>
                      ...
                      <ul className="nav nav-tabs mb-4">
                        ...
                        <li className="nav-item">
                          ...
                          >
                            {t('student_dashboard.cancelled_tab')} ({stats.cancelled})
                          </button>
                        </li>
                      </ul>
                        ...
                        <div className="lessons-list">
                          ...
                          {activeTab === 'cancelled' && cancelledBookings.length === 0 && (
                            <div className="text-center py-5">
                              ...
                              <p className="text-muted mt-3">{t('student_dashboard.no_cancelled_lessons')}</p>
                            </div>
                          )}
                                      ...
                                      <div className="d-flex gap-2 justify-content-end align-items-center">
                                        {booking.meeting?.joinUrlStudent && (
                                          ...
                                          >
                                            ...
                                            {t('student_dashboard.join_button')}
                                          </a>
                                        )}
                                        <div className="position-relative booking-action-menu">
                                          <button
                                          ...
                                          {openDropdownId === `${booking._id}_list` && (
                                            <div className="dropdown-menu show" style={{ position: 'absolute', right: 0, top: '100%', marginTop: '5px', zIndex: 1000, minWidth: '180px', boxShadow: '0 5px 15px rgba(0,0,0,0.1)', padding: '8px 0', border: 'none', borderRadius: '8px' }}>
                                              ...
                                              >
                                                ...
                                                <span>{t('student_dashboard.reschedule')}</span>
                                              </button>
                                              <button
                                                className="dropdown-item d-flex align-items-center gap-2 px-3 py-2 text-danger"
                                              ...
                                              >
                                                ...
                                                <span>{t('student_dashboard.cancel_lesson')}</span>
                                              </button>
                                            </div>
                                          )}
                                        </div>
                                ...
                                <div className="card-body p-3">
                                  <div className="row align-items-center">
                                    ...
                                    <div className="col-md-3 text-md-end">
                                      <span className="badge bg-secondary">{t('student_dashboard.status_completed')}</span>
                                    </div>
                                  </div>
                                </div>
                              </div>
                              ...
                              <div key={booking._id} className="card mb-3 border">
                                <div className="card-body p-3">
                                  <div className="row align-items-center">
                                    ...
                                    <div className="col-md-3 text-md-end">
                                      <span className="badge bg-danger">{t('student_dashboard.status_cancelled')}</span>
                                    </div>
                                  </div>
                                </div>
                              </div>
